...
object TrojanTranslator {
    ...
    /**
     * Java-callable callback. [kotlin.Result] is a value class and its onSuccess/onFailure
     * extension functions are not reachable from Java, so the hook code gets this instead:
     * exactly one of [text] / [error] is non-null.
     */
    fun interface ResultCallback {
        fun onResult(text: String?, error: Throwable?)
    }

    /** Java-friendly entry point — use this from TranslateAlert2.java and friends. */
    @JvmStatic
    fun translate(text: String, from: String?, to: String, cb: ResultCallback) {
        translate(text, from, to) { r -> cb.onResult(r.getOrNull(), r.exceptionOrNull()) }
    }
